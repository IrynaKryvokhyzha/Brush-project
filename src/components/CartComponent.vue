				...
				<div v-for="itemId in getCartList" :key="itemId" class="page__item">
					<div class="cart__products products">
						...
						<div class="products__product-content content-product">
							...
							<div class="content-product__item-price">({{getCurrentPrice(getItemPrice(itemId.id))}} {{ getCurrencyTitle }} / {{$t('cart.item')}}) </div>
							<button class="button-remove"	variant="text" @click="rejectItem(itemId.id)"> {{$t('cart.remove')}}	</button>
						</div>
					</div>
				</div>
				<div class="page__actions">
					<div class="subtotal">{{$t('cart.subtotal')}}: {{getTotalPrice()}} {{ getCurrencyTitle }}</div>
				</div>
				<div class="page__checkout-buttons">
					<button class="button-white" @click="closeCart">{{$t('button.keepShopping')}}</button>
					<button class="button-black">{{$t('button.checkout')}}</button>
				</div>
			</article> 
		</section> 
</template>
	...
	export default {
	 ...
	 methods: {
		...
		getItemImage(itemId){
			...
			const kitItem = this.getTravelKitItemsList.find(item => item.id == itemId)
			if (brush) {
				return brush.url;
			} else if (setItem) {
				return setItem.url;
